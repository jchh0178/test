...
<body>
    ...
    <%@ include file="../main/header.jsp" %>
		...
		<c:forEach var="list" items="${list2 }">
            <c:if test="${theaterDTO.region eq list.region }"  >
           		 <button class="active"><a href="${pageContext.request.contextPath}/theater/thtdetail2?name=${list.region}">${list.region}</a></button>
            </c:if>
            
            <c:if test="${theaterDTO.region ne list.region }"  >
            		<button><a href="${pageContext.request.contextPath}/theater/thtdetail2?name=${list.region}">${list.region}</a></button>
            </c:if>
        ...
        <!-- 지점명 -->
        <div class="branch-name" id="branchName">${theaterDTO.name }점</div>
